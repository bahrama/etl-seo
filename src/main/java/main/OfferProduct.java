package main;

import com.google.gson.Gson;
import com.mashape.unirest.http.HttpResponse;
import com.mashape.unirest.http.Unirest;
import model.product.Product;
import model.product.Variant;
import org.openqa.selenium.By;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.WebElement;
import org.openqa.selenium.firefox.FirefoxDriver;

import java.sql.Connection;
import java.sql.DriverManager;
import java.sql.ResultSet;
import java.sql.Statement;
import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

public class OfferProduct {
    public static void offerProductImport(String pidInp) {
        String token = "API@CJ3913573@CJ:eyJhbGciOiJIUzI1NiJ9..MlgRAobwrG_UA7voWVPaCP1sDDW3Rcwpo6Lxl-EcTRg";
        String token2 = "API@CJ3913891@CJ:eyJhbGciOiJIUzI1NiJ9..6cpSt8z9CPrKM8cQowCoSIopGWHuod9qUtA05gN7EfA";
        String token3 = "API@CJ3914046@CJ:eyJhbGciOiJIUzI1NiJ9..CraSmFdWuPrdroMlu8-AnVPnazF8XZKzDap-88Of5ew";
        String token4 = "API@CJ3914565@CJ:eyJhbGciOiJIUzI1NiJ9..1UkLxzBxdG6FcjH2Eie_lLyWIZWESB0gMjlwr-6RM3s";
        String token5 = "API@CJ3914579@CJ:eyJhbGciOiJIUzI1NiJ9..iE7tyALbp1G2tURibWzP-VE_JQTDYf6yjNM8vgJIAks";
        String token6 = "API@CJ3914589@CJ:eyJhbGciOiJIUzI1NiJ9..2yNfoK1YHF-c6dBaY5iFYRchngLDvlBwt4lwekf9TDw";
        String token7 = "API@CJ3914589@CJ:eyJhbGciOiJIUzI1NiJ9..qNXo1G5oxOcE17V4Ne_spBkdHE_687cbx-jEG2ZDNHU";

        String token8 = "API@CJ3914605@CJ:eyJhbGciOiJIUzI1NiJ9..BFGO-IlwjyZzYh3TANoZN4Y0vwdrWRGxLrJPy5iYrGg";
        String token9 = "API@CJ3983152@CJ:eyJhbGciOiJIUzI1NiJ9..MrUeKWKEGpWvnJYYSZ9V9WOYG_kIHGbUN7kAFBxcz3M";
        String token10 = "API@CJ3983169@CJ:eyJhbGciOiJIUzI1NiJ9.._BLEdI1xvf8OL86LZL4rW7IvRZXLZxqCl9R_qXhU1b0";
        String token11 = "API@CJ3983176@CJ:eyJhbGciOiJIUzI1NiJ9..sYdTDsbMthjZxAl5wMMQMW5_5bO4H4rcRTh0m8VBxTs";
        String token12 = "API@CJ3983231@CJ:eyJhbGciOiJIUzI1NiJ9..n6bC6HV8_F2nU-QOoWQ-zcGl4gBhWvrLbgHQjZnaKWc";
        String token13 = "API@CJ3983245@CJ:eyJhbGciOiJIUzI1NiJ9..N2LGfGbCWsjdgdm5Wp4694jkH2M0a3MXPBuCf2olykw";
        String token14 = "API@CJ3983287@CJ:eyJhbGciOiJIUzI1NiJ9..8FgLu1R1nj-mcuo6_N_L1_j-uaSenauvaGxppuf8LAg";
        String token15 = "API@CJ3983292@CJ:eyJhbGciOiJIUzI1NiJ9..VWdGc7Zx0PEUDbE-v_571vQy-hsGhNoMLhSHO8t-eD8";
        String token16 = "API@CJ3983298@CJ:eyJhbGciOiJIUzI1NiJ9..KuDXE20bkTGz8dmITHwfAde_925gQ78D7lFqqSNn5xQ";
        String token17 = "API@CJ3983972@CJ:eyJhbGciOiJIUzI1NiJ9..qxgN_aGg3HmgF_DrfO4Q-cmtM65HXX8EaPoAl5WnhFY";
        String token18 = "API@CJ3983978@CJ:eyJhbGciOiJIUzI1NiJ9..nvqofRUIpACK9Ymn30WWW15dpjLw6jC4E-Jy-54rpnc";
        String token19 = "API@CJ3983984@CJ:eyJhbGciOiJIUzI1NiJ9..rlqoiUcygzgBlaNCmE59_BSt42hqQRrytUfxh35ZuUM";

        String token20 = "API@CJ3984102@CJ:eyJhbGciOiJIUzI1NiJ9..sZswHQBOgxV2QO5B28uRCTZrZKInzSWIsepkkpmjSZs";
        String token21 = "API@CJ3984107@CJ:eyJhbGciOiJIUzI1NiJ9..NZzFMnpJlRjzGThKANqKA742n2fd3QhcijAKXZgy1eA";
        String token22 = "API@CJ3984112@CJ:eyJhbGciOiJIUzI1NiJ9..BbmBnlmMjHYOVbaEf43lLtw4bwwzKAo9L9JQP1IhcR0";

        String token23 = "API@CJ3984164@CJ:eyJhbGciOiJIUzI1NiJ9..sh6mL1gTQXEjZUvz6ZPBv9ZygcTen6D2M1NMemTAlBE";
        String token24 = "API@CJ3984181@CJ:eyJhbGciOiJIUzI1NiJ9..VF-5r9R6mc5FzbQIyGmhpKTs0lxWIBqutv8HIDXiX48";
        String token25 = "API@CJ3984191@CJ:eyJhbGciOiJIUzI1NiJ9..n26GceieIBJML45Xq_NYEAv_ZKucV8ugm4Ts_41aGLg";

        String token26 = "API@CJ3984552@CJ:eyJhbGciOiJIUzI1NiJ9..TF96Sg39WbSmcd-QKxyHhQNKpNhGCwYpYxmalbTRXw0";
        String token27 = "API@CJ3984539@CJ:eyJhbGciOiJIUzI1NiJ9..tO0X7VseHU1JZFyhqfawgTPX50yOCt0P58B1OLYBQ2g";
        String token28 = "API@CJ3984526@CJ:eyJhbGciOiJIUzI1NiJ9..k0ZpPEzIomBKMXQ-WQW1h3JAoHBXxz0HKZ9VoFRHQVc";
       // while (true) {
            try {
                int i = 0;
                Statement stmt = null;
                while (true) {
                    stmt = getStatement();
                    if (stmt == null) {
                        resetModem();
                        try {
                            Thread.sleep(30000);
                        } catch (InterruptedException e) {
                            throw new RuntimeException(e);
                        }
                    } else {
                        break;
                    }
                }
                try {
                    List<String> allTokens = new ArrayList<>();
                    allTokens.add(token);
                    allTokens.add(token2);
                    allTokens.add(token3);
                    allTokens.add(token4);
                    allTokens.add(token5);
                    allTokens.add(token6);
                    allTokens.add(token7);
                    allTokens.add(token8);
                    allTokens.add(token9);
                    allTokens.add(token10);
                    allTokens.add(token11);
                    allTokens.add(token12);
                    allTokens.add(token13);
                    allTokens.add(token14);
                    allTokens.add(token15);
                    allTokens.add(token16);
                    allTokens.add(token17);
                    allTokens.add(token18);
                    allTokens.add(token19);
                    allTokens.add(token20);
                    allTokens.add(token21);
                    allTokens.add(token22);
                    allTokens.add(token23);
                    allTokens.add(token24);
                    allTokens.add(token25);
                    allTokens.add(token26);
                    allTokens.add(token27);
                    allTokens.add(token28);
                    Collections.reverse(allTokens);
/*                    ResultSet rs = stmt.executeQuery("select pm.id , pm.pid from product_mapping pm left join offer_product op on pm.pid = op.pid where op.pid is null order by id asc limit 40000;");
                    List<String> productMappingPids = new ArrayList<>();
                    while (rs.next()) {
                        String pid = rs.getString("pid");
                        productMappingPids.add(pid);
                    }*/

/*                    for (String pid : productMappingPids
                    ) {*/
                        Gson gson = new Gson();
                        Thread.sleep(1000);
                        //Unirest.setTimeouts(0, 0);
                        HttpResponse<String> response = null;
                        Product product = null;
                        try {
                            response = Unirest.get("https://developers.cjdropshipping.com/api2.0/v1/product/query?pid=" + pidInp)
                                    .header("CJ-Access-Token", allTokens.get(i))
                                    .asString();
                            //if (response == null) continue;
                            System.err.println(response.getBody());
                            product = gson.fromJson(response.getBody(), Product.class);
                        } catch (Exception e) {
                            e.printStackTrace();
                            product = null;
                            Thread.sleep(5000);
                        }
/*                        if (product == null) {
                            while (true) {
                                resetModem();
                                stmt = getStatement();
                                if (stmt == null) {
                                    resetModem();
                                    try {
                                        Thread.sleep(30000);
                                    } catch (InterruptedException e) {
                                        throw new RuntimeException(e);
                                    }
                                } else {
                                    break;
                                }
                            }
                            i++;
                        }*/
                        if (product == null) {
                            System.out.println("product is null");
                           /// continue;
                        }
/*                        if (product.getMessage().contains("Product has been removed from shelves")) {
                            String sql = "delete from product_mapping where pid='" + pid + "';";
                            stmt.executeUpdate(sql);
                        }*/
                        //if (product.getMessage().contains("Too much request, QPS limit is 1 time/1second")) continue;
                        //if (product.getMessage().contains("Product has been removed from shelves")) continue;
                        if (product.getMessage().contains("Too much request")) {
                            while (true) {
                                resetModem();
                                stmt = getStatement();
                                if (stmt == null) {
                                    resetModem();
                                    try {
                                        Thread.sleep(30000);
                                    } catch (InterruptedException e) {
                                        throw new RuntimeException(e);
                                    }
                                } else {
                                    break;
                                }
                            }
                            i++;
                        }
                        if (product.getData() == null) {
                            System.out.println("data is null");
                           // continue;
                        }
                        if (product.getData().getEntryNameEn() == null) {
                            product.getData().setEntryNameEn("");
                        }
                        if (product.getData().getEntryCode() == null) {
                            product.getData().setEntryCode("");
                        }
                        if (product.getData().getDescription() == null) {
                            product.getData().setDescription("");
                        }
                        if (product.getData().getProductKeyEn() == null) {
                            product.getData().setProductKeyEn("");
                        }
                        if (product.getData().getDescription() == null) {
                            product.getData().setDescription("");
                        }
                        String sql = "insert into offer_product (add_mark_status, create_date, creater_time, description,\n" +
                                "                           entry_code, entry_name, listed_num, material_key,\n" +
                                "                           material_name, packing_key, packing_name, packing_weight, pid,\n" +
                                "                           product_all_image, product_image, product_key, product_name, product_pro,\n" +
                                "                           product_sku, product_type, product_video, product_weight, sell_price,\n" +
                                "                           short_description, source_from, sta_tus, suggest_sell_price, supplier_id,\n" +
                                "                           supplier_name, title, update_date, offer_id) VALUES " +
                                "(" +
                                "'"
                                + product.getData().getAddMarkStatus() +
                                "',"
                                + null +
                                "," +
                                ""
                                + null +
                                "," +
                                "'"
                                + product.getData().getDescription().replace("'", "''") +
                                "'," +
                                "'"
                                + product.getData().getEntryCode().replace("'", "''") +
                                "'," +
                                "'"
                                + product.getData().getEntryNameEn().replace("'", "''") +
                                "'," +
                                "'"
                                + product.getData().getListedNum() +
                                "'," +
                                "'"
                                + product.getData().getMaterialKey() +
                                "'," +
                                "'"
                                + product.getData().getMaterialNameEn() +
                                "'," +
                                "'"
                                + product.getData().getPackingKey() +
                                "'," +
                                "'"
                                + product.getData().getPackingNameEn() +
                                "'," +
                                "'"
                                + product.getData().getPackingWeight() +
                                "'," +
                                "'"
                                + product.getData().getPid() +
                                "'," +
                                "'"
                                + product.getData().getProductImage() +
                                "',"
                                + null +
                                "," +
                                "'"
                                + product.getData().getProductKeyEn().replace("'", "''") +
                                "'," +
                                "'"
                                + product.getData().getProductNameEn().replace("'", "''") +
                                "'," +
                                "'"
                                + product.getData().getProductProEn() +
                                "'," +
                                "'"
                                + product.getData().getProductSku() +
                                "'," +
                                "'"
                                + product.getData().getProductType() +
                                "'," +
                                "'"
                                + product.getData().getProductVideo() +
                                "'," +
                                "'"
                                + product.getData().getProductWeight() +
                                "'," +
                                "'"
                                + product.getData().getSellPrice() +
                                "'," +
                                "'"
                                + "" +
                                "'," +
                                "'"
                                + product.getData().getSourceFrom() +
                                "'," +
                                "'"
                                + product.getData().getStatus() +
                                "'," +
                                "'"
                                + product.getData().getSuggestSellPrice() +
                                "'," +
                                "'"
                                + product.getData().getSupplierId() +
                                "'" +
                                ",'"
                                + product.getData().getSupplierName() +
                                "'," +
                                "'"
                                + "" +
                                "',"
                                + null +
                                ","
                                + null +
                                ")";
                        try {
                            stmt.executeUpdate(sql);
                            System.out.println("product added");
                        } catch (Exception e) {
                            System.out.println("product error");
                            e.printStackTrace();
                        }

                        for (Variant variant : product.getData().getVariants()
                        ) {
                            if (variant.getVariantNameEn() == null) variant.setVariantNameEn("");
                            String sqlVar = "insert into offer_product_variant\n" +
                                    "    (create_date, create_time, pid, update_date, variant_height, variant_image, variant_key, variant_length, variant_name, variant_property, variant_sell_price, variant_sku, variant_standard, variant_sug_sell_price, variant_unit, variant_volume, variant_weight, variant_width, vid, offer_product_id)\n" +
                                    "values" +
                                    "("
                                    + null +
                                    ","
                                    + null +
                                    "," +
                                    "'"
                                    + variant.getPid() +
                                    "',"
                                    + null +
                                    "," +
                                    "'"
                                    + variant.getVariantHeight() +
                                    "'," +
                                    "'"
                                    + variant.getVariantImage() +
                                    "'," +
                                    "'"
                                    + variant.getVariantKey().replace("'", "''") +
                                    "'," +
                                    "'"
                                    + variant.getVariantLength() +
                                    "'," +
                                    "'"
                                    + variant.getVariantNameEn().replace("'", "''") +
                                    "'," +
                                    "'"
                                    + variant.getVariantProperty() +
                                    "'," +
                                    "'"
                                    + variant.getVariantSellPrice() +
                                    "'," +
                                    "'"
                                    + variant.getVariantSku() +
                                    "'," +
                                    "'"
                                    + variant.getVariantStandard() +
                                    "'," +
                                    "'"
                                    + variant.getVariantSugSellPrice() +
                                    "'," +
                                    "'"
                                    + variant.getVariantUnit() +
                                    "'," +
                                    "'"
                                    + variant.getVariantVolume() +
                                    "'," +
                                    "'"
                                    + variant.getVariantWeight() +
                                    "'," +
                                    "'"
                                    + variant.getVariantWidth() +
                                    "'," +
                                    "'"
                                    + variant.getVid() +
                                    "'," +
                                    ""
                                    + null +
                                    "" +
                                    ")";
                            try {
                                stmt.executeUpdate(sqlVar);
                                System.out.println("variant added");
                            } catch (Exception e) {
                                System.out.println("variant error");
                                e.printStackTrace();
                                String sql3 = "delete from offer_product_variant where vid='" + variant.getVid() + "';";
                                stmt.executeUpdate(sql3);
                                System.out.println("variant deleted");
                            }
                        }
                    //}
                    stmt.close();
                } catch (Exception e) {
                    System.err.println(e.getClass().getName() + ": " + e.getMessage());
                    System.exit(0);
                }
            }catch (Exception e){
                try {
                    Thread.sleep(30000);
                } catch (InterruptedException ex) {
                    throw new RuntimeException(ex);
                }
            }
        //}
    }
    public static Statement getStatement () {
        Connection c = null;
        Statement stmt = null;
        try {
            Class.forName("org.postgresql.Driver");
            c = DriverManager
                    .getConnection("jdbc:postgresql://159.69.242.158:5432/postgres",
                            "postgres", "ali680313");
            //c.setAutoCommit(false);
            System.out.println("Opened database successfully");
            stmt = c.createStatement();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return stmt;
    }

    public static void resetModem () {
        WebDriver driver = new FirefoxDriver();
        try {
            System.out.println("start");
            driver.get("http://192.168.1.1/login.html");
            WebElement emailInput = driver.findElement(By.id("login_uname"));
            emailInput.clear();
            //Enter Text
            String username = "admin";
            emailInput.sendKeys(username);
            WebElement emailInput2 = driver.findElement(By.id("login_password"));
            emailInput2.clear();
            //Enter Text
            String pass = "Taknet15431543";
            emailInput2.sendKeys(pass);
            Thread.sleep(1000);
            WebElement submit = driver.findElement(By.id("btn_login"));
            submit.click();
            Thread.sleep(1000);
            WebElement setting = driver.findElement(By.id("menuSettings"));
            setting.click();
            Thread.sleep(1000);
            WebElement reboot = driver.findElement(By.id("submenu_sys99"));
            reboot.click();
            Thread.sleep(1000);
            WebElement ckeck = driver.findElement(By.id("reboot_checked_enable"));
            ckeck.click();
            Thread.sleep(1000);
            WebElement btnReboot = driver.findElement(By.id("btnReboot"));
            btnReboot.click();
            Thread.sleep(90000);
            driver.quit();
        } catch (Exception e) {
            e.printStackTrace();
            driver.quit();
            System.out.println("an error");
        }
    }
}
